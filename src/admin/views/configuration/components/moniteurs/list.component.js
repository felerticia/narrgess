...
class List extends Component {
    ...
    UNSAFE_componentWillMount() {
        getMoniteurs().then(data => this.setState({ data, moniteurs: data, loading: false }));
    }

    handleChangePayload(payload) {
        let moniteurs = {};
        if (payload.length < 1) {
            moniteurs = this.state.data;
        } else {
            ...
            keys.forEach(key => moniteurs[key] = this.state.data[key]);
        }
        this.setState({ payload, moniteurs });
    }
        ...
        return (
            <div>
                ...
                <Table singleLine striped selectable fixed textAlign='center'>
                    ...
                    <Table.Body>
                    {
                        this.state.moniteurs &&
                        Object.keys(this.state.moniteurs) &&
                        Object.keys(this.state.moniteurs).map(key => (
                            <Table.Row key={key} onClick={() => this.props.history.push(`/admin/configuration/moniteurs/${key}`)}>
                                <Table.Cell>{this.state.moniteurs[key].nom}</Table.Cell>
                                <Table.Cell>{this.state.moniteurs[key].prenom}</Table.Cell>
                                <Table.Cell>{this.state.moniteurs[key].ville}</Table.Cell>
                                <Table.Cell>{this.state.moniteurs[key].tel}</Table.Cell>
                            </Table.Row>
                        ))
                    }
                    </Table.Body>
